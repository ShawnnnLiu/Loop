...
def test_replan_continuation_carries_plan_diff_and_draft_view_projection() -> None:
    """A replan continuation carries the deterministic old→new content diff
    (D4 stage 2) on the result AND in the draft projection; a fresh propose
    carries neither (no parent plan to diff against). The fixture planner
    reproduces identical content, so the diff honestly reports zero changes."""
    payload = {**_motivation_profile_payload(), "recovery_mode_preference": "ask_each_time"}
    service, env, clock = make_service(motivation_profile=payload)

    proposed = _activate_plan(service)
    assert proposed.plan_diff is None  # fresh propose: nothing to diff against
    assert service.draft_view(USER_ID).plan_diff is None
    _advance_past_draft(env, clock, proposed.draft_schedule_id)

    service.ingest(USER_ID, [_missed_event("evt_001", "dp_001")])
    continuation = service.propose(USER_ID, recovery_mode=RecoveryAction.SCOPE_REDUCTION)
    assert continuation.state is S.AWAITING_USER_APPROVAL
    assert continuation.plan_diff is not None
    assert continuation.plan_diff.from_plan_version == proposed.plan_version
    assert continuation.plan_diff.to_plan_version == continuation.plan_version

    view = service.draft_view(USER_ID)
    assert view.plan_diff is not None
    assert view.plan_diff.from_plan_version == proposed.plan_version
    assert view.plan_diff.to_plan_version == continuation.plan_version
    assert view.plan_diff.tasks_preserved == 2
    assert view.plan_diff.tasks_changed == 0
    assert view.plan_diff.tasks_added == 0
    assert view.plan_diff.tasks_removed == 0
    assert view.plan_diff.changes == []
