...
### Serving curation floor (UX-pass D1b; retuned 2026-07-14)

The pre-prompt curation filter (`source_claims/curation.py`, tuning section
`claim_curation`) drops stored claims below `min_confidence` before they are
serialized into the Strategist prompt. Change record, axiom 07 style:

| Field | Value |
| --- | --- |
| `threshold_field` | `claim_curation.min_confidence` (code default) |
| `prior_value` | 0.30 |
| `new_value` | 0.25 |
...
deployment that wants the old floor pins `min_confidence = 0.30` there, which
journals normally. The floor remains a heuristic prior: it tracks the uniform
scores of today's store (no published dates, no corroboration groups), not
