...
A task with unmet prerequisites must not be scheduled before its blockers unless the task is explicitly marked as a preview-only future placeholder. For the MVP, blocked tasks are not scheduled at all and the Scheduler emits `DEPENDENCY_BLOCKED` if asked.
