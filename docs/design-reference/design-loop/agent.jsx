// Agent — hi-fi (Loop · interview-prep & job-search scheduler)
// Docked right rail (never a floating pill). Approvals → thread → composer + trust strip.
// Plus a capability map separating AI-proposes from deterministic-acts.
    ...
    <aside className="dock">
      ...
      <div className="dock-section">
        ...
        <div className="col" style={{gap:8}}>
          {[
            {t:'Graphs · Dijkstra drill', m:'Wed 10:00 · 1h focus'},
      ...
      <div className="dock-section grow">
        <div className="label" style={{marginBottom:11}}>Thread · graphs milestone</div>
        <div className="chat" style={{flex:1, justifyContent:'flex-end', overflow:'hidden'}}>
          <div className="bubble agent">Morning. Graphs is the only thing between you and Friday's DSA-core milestone — it's your weakest topic and it's all over your target companies' banks.</div>
          <div className="bubble agent">I drafted a 1h Dijkstra drill at 10:00 — before your café shift, when you focus best. <b>Review above ↑</b></div>
          <div className="bubble me">Can you pull the 3 graph problems I got wrong last week?</div>
          <div className="bubble tool">⌁ read_file(attempts-log.json) · 1.6k tokens</div>
          <div className="bubble agent">Found them — two on shortest paths, one on topological sort. They'll seed the drill when you start the block; nothing's added until you do.</div>
        </div>
      </div>
...
function CapabilityMap() {
  ...
  const llm = [
    {icon:'¶', title:'Parse résumé / files', desc:'Reads your CV and attempt logs. Extracts stack, experience, weak spots, target companies — you review every field.'},
    {icon:'⚑', title:'Generate prep plan', desc:'Turns goal + skill profile into a milestone tree (DSA, system design, behavioral). Always lands in the approval gate first.'},
    ...
    {icon:'?', title:'Explain "why this"', desc:'Natural-language reasoning for any block — weak-spot priority, company fit, energy fit.'},
    {icon:'∑', title:'Mock & reflection', desc:'Drafts mock-interview questions and weekly summaries of what got done and what slipped.'},
  ];
  const det = [
    {icon:'▷', title:'Accept & schedule', desc:'Writes accepted blocks to Google Calendar. Idempotent, retry-safe, no model in the path.'},
    {icon:'✓', title:'Mark done', desc:'Logs completion + actual duration + problems solved. Feeds calibration and milestone progress.'},
    {icon:'⊘', title:'Drift detection', desc:'Rule-based: missed % + reschedule count → behind / on-track / ahead.'},
    {icon:'⤓', title:'Calendar sync', desc:'Pulls busy windows (shifts, classes), respects quiet hours, never silently mutates an event.'},
    {icon:'⚐', title:'Permission gate', desc:'Mentor / peer visibility and check-in reports — explicit, per-field consent.'},
