...
**Status (2026-07-19):** `tools/enrich_taxonomy.py` exists (RI-F,
`../resume-intake-onboarding/06-skill-taxonomy.md`); counting semantics
are pinned in `docs/specs/skill-taxonomy.schema.md` § "Enrichment
semantics" — phrase queries, track-scoped, distinct-chunk union, and the
per-alias report breakdown recommended below. Careers can still land
through step D without running it — enrichment annotates, it never
blocks.
