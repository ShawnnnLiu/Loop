...
**completed**, **skipped**, or **dropped**. Before this record existed the system
...
| `disposition` | enum | `completed`, `skipped`, or `dropped`. |
...
- Recorded by `planning/drop.py` (`dropped`) and the `ingest` completion mirror
  (`completed`).
